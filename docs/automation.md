...
> **Status: the Cloud Routine is disabled, blocked on a second live fire.** The `fps-sk0` blocker (`bd dolt push` couldn't authenticate from a Routine sandbox, [#384](https://github.com/edwinsteele/fuel-price-signal/issues/384), closed as moot) is gone — `bd` is deleted. The first live fire after re-enabling (2026-09-08) surfaced a *different* blocker: the Routine's environment token serves only a pinned set of PR-review GraphQL operations and 403s on everything else, which is what `gh issue/pr list/view --json` and `gh issue/pr edit` resolve through. CLAUDE.md's pickup rules are now rewritten to use REST instead, but that rewrite is untested against the real restricted token. Re-enabling is phase 7 of [#398](https://github.com/edwinsteele/fuel-price-signal/issues/398) and needs a second live fire to confirm. The mechanics below describe the intended flow but the worker-Routine specifics live in CLAUDE.md, which is current.
