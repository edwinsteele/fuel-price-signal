...
> **Status: the Cloud Routine is enabled and confirmed working.** Phase 7 of [#398](https://github.com/edwinsteele/fuel-price-signal/issues/398) is done. The `fps-sk0` blocker (`bd dolt push` couldn't authenticate, [#384](https://github.com/edwinsteele/fuel-price-signal/issues/384), closed as moot) is gone with `bd` deleted; a first live fire after re-enabling (2026-09-08) hit a *different* blocker — the Routine's environment token serves only a pinned set of PR-review GraphQL operations and 403s on everything else — fixed by rewriting CLAUDE.md's pickup rules to use REST throughout. A second live fire the same day confirmed the rewrite end to end: claimed an issue, opened a PR via REST, checked for reviews, and merged clean. The mechanics below describe the intended flow but the worker-Routine specifics live in CLAUDE.md, which is current.
