...
| `tgp.py` | Partial | AIP Sydney TGP downloader → `data/tgp/tgp_sydney.csv`; daily `tgp-fetch.yml` action. `tgp_delta_7d` is computed into every features.csv row (chips 1–3 of #271 done) but deliberately held OUT of `FEATURE_COLUMNS`/the trained model. The AI-sourced pipeline's batch0 re-test (2026-08-18) appeared to reject it (+0.0941, wrong sign), but that run's R0 was not the production baseline — it carried the 10 rejected Phase 4b brand-trough columns (bd fps-sa1, PR #309). Corrected rerun 2026-08-19 on the same frozen snapshot with the locked 54-column baseline: pooled realised delta_cpl_held **+0.0059 c/L — inert**, neither the June graduation (−0.039) nor a rejection. All three numbers sit within a couple of decision flips of zero (one flip ≈ 0.05 c/L pooled), so the realised arbiter cannot resolve this effect. `#271`'s remaining chips (`FEATURE_COLUMNS` bump + `decide()` wiring) stay on **hold**; the unblocking step is a noise floor (`fps-3jj.9`), not another paired run. Full detail: `experiments/candidates/batch0/tgp_delta_7d/README.md`, bd `fps-nor`. |
