...
| `signal.py` | Done | Route-aware morning decision table (#410): ranks reachable stations by PRICE, scores `P(BUY)` live from the locked artifact, calls fill size from network drift, flags staleness against `prices` (not forward-filled `daily_prices`). Station order is deliberately never `P(BUY)` — that is station-relative, see [docs/memory/pbuy-is-station-relative.md](memory/pbuy-is-station-relative.md). The four-signal port and `combine_signals` are unchanged and still back `RuleBasedSignalStrategy`, but are CLI-visible only behind `--explain`. 47 unit + integration tests |
