...
| `signal.py` | Done | Route-aware morning decision table (#410): ranks reachable stations by PRICE, scores `P(BUY)` live from the locked artifact, calls fill size from network drift, flags staleness against `prices` (not forward-filled `daily_prices`). Station order is deliberately never `P(BUY)` — that is station-relative, see [docs/memory/pbuy-is-station-relative.md](memory/pbuy-is-station-relative.md). Split (#415) into `compute_signal(...) -> SignalPayload` (the expensive, day-stable half — a pure function of `as_of_date` and the DB, safe to cache) and `render_text(payload, routing_day=, now=, explain=)` (the cheap, day-sensitive half — reachability, diversion arithmetic, freshness age); `build_signals` remains as a one-shot wrapper for the CLI. See [docs/memory/signal-payload-day-stable-boundary.md](memory/signal-payload-day-stable-boundary.md) for the caching invariant, which is the precondition for #416's nightly-compute/per-request-render API server. The four-signal port and `combine_signals` are unchanged and still back `RuleBasedSignalStrategy`, but are CLI-visible only behind `--explain`. 65 unit + integration tests |
...
- **The re-lock is not a retrain and not a τ change.** The on-disk artifact and τ=0.25
  ...
  daily signal (`fuel_signal/signal.py`) already emits daily. (As of #410 that CLI
  scores the locked artifact live rather than running the rules; still no
  `TankParams` involved, so the cadence re-lock remains orthogonal to it.)
