...
how-we-do-things layer. Atomic gotchas about specific code paths, data and tools
live in [docs/memory/](memory/INDEX.md).
...
detail live in GitHub issues and `experiments/` dirs, cited by number — don't expect a
...
1. **Re-read the issue** if the change closes one (`gh issue view <N>`). Walk through the acceptance criteria / deliverables list and confirm each item is covered by the diff. Scope often drifts during implementation; the issue is the source of truth for what was promised, and the check catches gaps before review does. If something in the issue is no longer the right thing to build, say so in the PR body rather than silently dropping it. Once merged, close it explicitly (`gh issue close <N>`) unless the PR body's `Closes: #N` already did.
...
- **Cut PR branches from `origin/main`, never local `main`:** `git fetch origin && git checkout -b <branch> origin/main`. **Why:** this repo routinely leaves local `main` ahead of origin (lab-book entries and `results.csv` lock rows commit direct to `main`). Branching off local `main` bundles those unpushed commits into the PR's squash merge and diverges `main` from origin afterwards. Hit three times — PRs #256, #261, #304.
- **Don't leave commits unpushed across a work session.** **Why:** `pull.ff=only` is set here, so `batch_freeze.py` / `make update`'s plain `git pull` fails hard with "Diverging branches can't be fast-forwarded" as soon as the daily-snapshot GHA workflow pushes concurrently. `git fetch && git rebase origin/main && git push` *before* running anything that shells out to `make update`.
- **The remote branch is already gone after a merge.** `edwinsteele/fuel-price-signal` has "Automatically delete head branches" on, so don't treat a failing `git push origin --delete <branch>` as an error — check with `git ls-remote --exit-code origin <branch>` or just let it fail.
- **Merge from a worktree via the API, not `gh pr merge`.** `gh pr merge --delete-branch` fails with `fatal: 'main' is already used by worktree` whenever it runs from a worktree while another worktree has `main` checked out — it tries local branch housekeeping that git worktrees disallow across trees. Use `gh api repos/<owner>/<repo>/pulls/<N>/merge -X PUT -f merge_method=squash`, which never touches local git, and skip `--delete-branch` entirely. Confirm with `gh pr view <N> --json state,mergedAt,mergeCommit`.
- **`gh pr view <N> --json reviewThreads` errors here** ("Unknown JSON field"). Use `--json comments,reviews,mergeable,statusCheckRollup` for a first pass: `reviews` carries review bodies, `comments` is PR-level top comments. This `gh` version has no per-thread `isResolved` equivalent. **This alone is not enough to check Codex's status**, despite the claim in an earlier version of this line that `reviews` carries "inline comments embedded as markdown" — Codex's findings are actual inline review comments (`gh api repos/{owner}/{repo}/pulls/<N>/comments`), invisible to both `--json` fields above, and its clean pass can be a bare reaction with no comment/review/check-run at all. Use the `codex-pr-review` skill, or [docs/memory/codex-pr-review-integration.md](memory/codex-pr-review-integration.md) directly, whenever checking review status on this repo. Missing this reported PR #422 as reviewer-silent when Codex had actually cleared it via a `+1` reaction.
- **Never write to `fuel_signal.db` before the PR introducing that code is merged.** A `SELECT`-only query against the live DB is fine anytime; a *write* using unreviewed code is the same class of mistake as pushing unreviewed code. Smoke-test against a temp or in-memory DB instead. Exception: the user explicitly authorises a live-DB write for a specific purpose.
- **`experiments/results.csv` is modified only on `main`, and only after explicit user confirmation of the write step.** Never stage or commit it from a feature branch. Per-experiment-dir `results.csv` files (e.g. `experiments/l1_per_row_lga/results.csv`) are a separate, always-fine convention. **Schema changes count too** — asking is not optional just because no result row is being added. Precedent: adding the `baseline_fingerprint` column (PR #312) required migrating all 29 existing rows because `log_experiment` hard-fails when the on-disk header doesn't match `_CSV_HEADER` exactly; the owner approved that specific change on a branch, and that approval doesn't generalise. Two traps found doing it: three legacy rows carry unquoted commas in `notes`, so `DictReader` parses them into a `None` overflow key and the `DictReader`→`DictWriter` round-trip raises; and `csv.writer` defaults to CRLF while git normalises with `core.autocrlf=input`, which rewrote all 30 lines as invisible churn (now pinned by `.gitattributes` `eol=lf` plus `lineterminator="\n"`). Verify edits to it with `read_bytes()`, not `splitlines()` — `splitlines()` strips both terminators and cannot see the flip.
...
This repo's worktrees live *inside* the primary checkout at `.claude/worktrees/*`, which makes several ordinary-looking git operations behave unexpectedly.

- **Prefer cwd-relative paths inside a worktree.** Editing a shared doc (`ledger.yaml`, `INDEX.md`, `CONVENTIONS.md`) by absolute path is how you silently edit the *primary* checkout's copy instead — typing the primary path from muscle memory dirties `main` rather than your branch, with no error. Caught live during PR #351 before it was committed.
