...
**The ICC is a BOUND, and deliberately the pessimistic end of one.** `placebo.TEXTURE_ICC_BOUND = 0.391` is the 95% upper limit from a one-way ANOVA of batch1's committed 20-draw floor grouped by texture family (`F(3,16) = 0.411`, `p = 0.75`, point estimate ≈ 0, but the design could not resolve below **0.359** — a "could not see it", never "it is not there"). Using the bound makes every bar a little harder than it probably needs to be, which is the safe direction: the failure this mechanism exists to prevent is a band too narrow and a bar too easy. bd `fps-3jj.23` (P1, ~2.3h) measures it directly; when it lands, replace the constant and every bar tightens. Note the family grouping is *coarser* than by column, so the bound likely **overstates** the correction — again the safe direction. Full working: `experiments/2026-08-26_placebo_draw_independence/`.
