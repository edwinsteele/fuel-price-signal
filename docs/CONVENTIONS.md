...
- **`gh pr view <N> --json reviewThreads` errors here** ("Unknown JSON field"). Use `--json comments,reviews,mergeable,statusCheckRollup`: `reviews` carries bot and human review bodies with inline comments embedded as markdown, `comments` is PR-level top comments. This `gh` version has no per-thread `isResolved` equivalent.
