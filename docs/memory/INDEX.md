...
- [pr-labels-not-inherited-from-issue](pr-labels-not-inherited-from-issue.md) — `gh pr create` doesn't copy the issue's labels; a `design` PR opened without `--label design` makes `signal-regression.yml`'s `signal-diff` job silently `skipped`, invisible unless you check every `check_runs[].conclusion`.
