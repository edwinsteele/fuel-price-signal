...
- **Post-merge git hygiene is standing-authorized — do it, don't ask.** Once `gh pr view <N> --json state` shows `MERGED` for a PR you opened in this session:
  - `git branch -D <branch>` the now-local-only branch. Squash-merge means git won't recognize it as an ordinary merge (`branch -d` will refuse), but the content is already in the squash commit on `main`, so force-deleting the local pointer loses nothing. The remote copy is usually already gone — this repo has "auto-delete head branches" on (`bd recall github-auto-deletes-merged-branch` if a manual delete surprises you with "remote ref does not exist").
  - `git pull --ff-only` any other worktree (including the primary one) that's now behind `main` — check `git status --short` is empty there first; a bare fast-forward on a clean tree can't lose anything.
  - Both are narrowly scoped to *this* repo's per-issue-worktree workflow and to branches/worktrees you've just confirmed are merged — they are not a general license for `branch -D`/force operations elsewhere.
