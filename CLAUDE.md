...
**Known gap, fix applied 2026-08-11 (pending live confirmation):** the Dolt remote was `git+ssh://git@github.com/edwinsteele/fuel-price-signal.git`, which needs deploy-key or HTTPS-token auth reachable from the Routine's environment; it relied on the owner's personal 1Password-gated SSH key, which that environment does not have. **Root cause confirmed live 2026-08-11:** the container has no `ssh` binary, and the environment's git `insteadOf` proxy rewrites only match `git@github.com:` and `ssh://git@github.com/` — not the `git+ssh://` scheme `bd` actually uses for `sync.remote` — so the rewrite never fired and the connection had no way to authenticate. **Fix (fps-ddf):** `sync.remote` now points at `https://github.com/edwinsteele/fuel-price-signal.git` (bd stores it internally as `git+https://…`) instead of `git+ssh://…`. This repo is public, so `bd dolt pull` needs no credential at all over https — confirmed. `bd dolt push` still needs a real https git credential: on the owner's Mac this required running `gh auth setup-git` once to give git an `https://github.com` credential helper backed by `gh`'s token (an earlier "verified locally" claim for push was a false positive — it happened to be a no-op push with nothing new to send, so it never touched auth). **Whether the Routine's container has an equivalent working https credential for git already is unconfirmed** — its `git push` for `worker/<id>-<slug>` branches works today without an ssh binary, which is circumstantial evidence some https/token path already exists there, but this needs a live RemoteTrigger run doing a real `bd dolt push` (not just pull) to confirm before this paragraph can be deleted.
