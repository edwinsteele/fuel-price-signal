...
**Known gap — pull works, push does not (re-diagnosed live 2026-08-11, sharper than the original framing below):** a live RemoteTrigger run against the *pre-fix* remote (`git+ssh://git@github.com/edwinsteele/fuel-price-signal.git`, since the https change below hadn't merged yet) found `bd bootstrap`/`bd dolt pull` **succeed** in the Routine's container — 944 chunks synced cleanly, no `ssh` binary needed. The run's own hypothesis: the container has a git `insteadOf` rule rewriting `ssh://git@github.com/` → an authenticated `https://…`, and bd's internal git client apparently strips the `git+` prefix before invoking it, so the rewrite does fire despite the literal `sync.remote` string starting with `git+ssh://`. So the original "insteadOf doesn't match the scheme" theory below was likely wrong, or at least incomplete.

**The real, still-open blocker is `bd dolt push`**, which failed with `HTTP 403` in that same run — even though an ordinary `git push --dry-run` in the identical container succeeded immediately with valid credentials. So the container's git-level push auth works fine; bd's internal client isn't picking it up for the Dolt data push specifically. `bd dolt push --help` documents `DOLT_REMOTE_USER`/`DOLT_REMOTE_PASSWORD` env vars for HTTPS basic auth — undocumented whether these apply outside "Hosted Dolt" remotes, but this is the most likely supported mechanism, and neither var is currently set in the Routine's environment. Fixing this needs an owner decision (a GitHub PAT as `DOLT_REMOTE_PASSWORD`, scoped and stored as a Routine secret — not committed to any tracked file) — not something to pick unilaterally. Until it's set up, treat `bd dolt push` (and therefore issue claims, since a claim that can't push is invisible to every other clone) as unreliable in the Routine, even though `bd dolt pull` works.

<details><summary>Original framing (2026-08-11, superseded by the re-diagnosis above — kept for history)</summary>

Fix applied (fps-ddf): `sync.remote` now points at `https://github.com/edwinsteele/fuel-price-signal.git` (bd stores it internally as `git+https://…`) instead of `git+ssh://…`, on the theory that the container's `insteadOf` rewrites didn't cover the `git+ssh://` scheme. The live run above shows pull was never actually broken by that scheme mismatch — and since push fails for an unrelated credential-plumbing reason in bd's internal client, this URL-scheme change alone is unlikely to fix push. Keeping the https remote anyway (it's at least no worse, and pull working "for a different reason than we thought" is still pull working), but the push blocker needs the `DOLT_REMOTE_USER`/`PASSWORD` fix described above, not a URL change.
