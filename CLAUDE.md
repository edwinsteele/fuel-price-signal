...
**Pickup rules:**
0. Ensure `bd` is usable and version-pinned before anything else: `command -v bd >/dev/null 2>&1 || npm install -g @beads/bd@1.1.2`. If npm itself isn't available in the container, fall back to `curl -fsSL https://raw.githubusercontent.com/gastownhall/beads/main/scripts/install.sh | bash` — but note in your run output that this installs whatever is currently tagged `latest` upstream (the script has no version-pin option), so a fallback-installed `bd` is running unvalidated against the shared Dolt database rather than the pinned, owner-confirmed version. Then re-check `command -v bd`; if it's still missing, surface that clearly and stop — do not proceed as if there were no work. Once `bd` is confirmed present, run `bd dolt pull`.
